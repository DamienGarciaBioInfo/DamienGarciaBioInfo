...
station_request=$1 ;
station_output=$(tre-agrep -$((AMATCH_DISTANCE)) -i -s $station_request 244400404_stations-velos-libre-service-nantes-metropole.csv | sort -n | head -n 1 | cut -d ';' -f 2 | tr -d -c '0-9');
echo $station_output > "station_id.txt"
